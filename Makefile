...
#GENCODE = -gencode=arch=compute_60,code=sm_60 # titan, gp100
GENCODE = -gencode=arch=compute_70,code=sm_70 # v100s
...
CCFLAGS := -O3 --ptxas-options=-v -Xptxas -dlcm=ca $(GENCODE) -DEARLY_EXIT=0 -DTHREADS_PER_BLOCK=$(NUM_THREADS) -DNUM_THREADS=$(NUM_THREADS) \
...
benchmark.o: main.cu main.h
	$(NVCC) $(CCFLAGS) -DMAX_HAMMING_DIST=8 -c -o $@ $< 
